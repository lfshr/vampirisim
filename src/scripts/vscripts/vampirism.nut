...
MutationState <- {
	...
	vampirismConsumeSpeed = 1.3 // How fast does the health decay build up to max?
	...
	infected = [
		{ zombietype="common", 	HealthPoints=3, 	grantAllSurvivorsHealth=false,	reviveIncapacitatedSurvivors=false }, 	// COMMON INFECTED = index 0
...
HELP_TEXT_ENG <- [
	...
	" Tip: Shoot fellow survivors to steal their health!"
]
...
function AddPlayerHealth(player, healthAdd){
	local healthNew = player.GetHealthBuffer();
	healthNew += healthAdd;
	...
	player.SetHealthBuffer( healthNew );
		...
		foreach( name, stats in SessionState.survivors ){
			...
			if( stats.vampirismEnabled ){
				...
				if( !survivor.IsIncapacitated() ){
					...
					healthToSet <- survivor.GetHealthBuffer() - stats.currentVampirism;
					if( healthToSet < 1 ) healthToSet = 1; // Lowest health can be 1 HP so if it's below 1 set it to 1
					survivor.SetHealthBuffer( healthToSet );// Set the health to the health decay
				}
...
function OnGameEvent_revive_success( args ){
	...
	player.SetHealthBuffer( SessionState.vampirismReviveHealth );
}
